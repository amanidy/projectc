...
```tsx
const x: T = { foo: 'bar', baz: 42 };
const y = { foo: 'bar', baz: 42 } as any;
const z = { foo: 'bar', baz: 42 } as unknown;
foo({ foo: 'bar', baz: 42 } as T);
new Clazz({ foo: 'bar', baz: 42 } as T);
function foo() { throw { bar: 5 } as Foo }
const foo = <Foo props={{ foo: 'bar', baz: 42 } as Bar}/>;
```
